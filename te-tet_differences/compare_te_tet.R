...
compare.count <- read_csv("data/raw_descriptions-2021.csv")
...
compare.count$difference[compare.count$difference == Inf] <- 4
...
compare.graph <- ggplot(compare.count, aes(x = reorder(abbreviations, difference), 
  ...
  theme_pubr() +
  scale_color_brewer(palette = "Dark2", name = "Manuscript Category") +
  #scale_color_manual("Manuscript Category", values = c("#01426A", "#DC8823")) + 
  scale_y_continuous(breaks = -4:4, limits = c(-4, 4), labels=c(">16X","8X","4X","2X","Same","2X","4X", "8X",">16X")) + xlab("Descriptors") +
...
tiff("results/compare-te_tet-2021.tif", width = 11, height = 8.5, units = "in", res = 300)
  #dev.set(3)
  print(compare.graph)
dev.off()
ggsave("results/compare-te_tet-2021.png", width = 11, height = 8.5, units = "in", dpi = 150)
